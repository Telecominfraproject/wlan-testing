...
class TestUcentralAnalyticService(object):
    ...
    def test_provservice_boardlist(self, setup_prov_controller, get_configuration):
        """
            Test the device present in Provisioning UI
        """
        device_name = get_configuration['access_point'][0]['serial']
        resp = setup_prov_controller.get_board_by_device(device_name)
        print(resp.json())
    ...
    def test_prov_service_create_edit_delete_board_device(self, setup_prov_controller, testbed):
        """
        ...
        resp = setup_prov_controller.add_device_to_board(device_name, payload)
