...
class Test_RatevsRange_Bridge(object):
    ...
    def test_rvr_bridge_dl_wpa2_personal_2g(self, setup_configuration, get_test_library, num_stations,
        ...
        val = [['modes: Auto'], ['pkts: MTU'], ['directions: DUT Transmit'], ['traffic_types:TCP'],
               ['bandw_options: AUTO'], ['spatial_streams: 2'], ['attenuator: ' + str(ser_no[1])], ['attenuator2: 0'],
        ...
        if station:
            rvr_o, report_name = get_test_library.rate_vs_range_test(station_name=sta_name[0], mode=mode, download_rate="100%",
                                                        duration='30000',instance_name="BRIDGE_RVR_TWOG_DL",vlan_id=vlan,
    ...
    @allure.testcase(url="https://telecominfraproject.atlassian.net/browse/WIFI-2495", name="WIFI-2495")
    def test_rvr_bridge_ul_wpa2_personal_2g(self, setup_configuration, get_test_library, num_stations,
                                         get_test_device_logs, get_dut_logs_per_test_case, check_connectivity):
        logging.info("Cleanup existing clients and traffic")
        chamber_view_obj, dut_name = get_test_library.chamber_view()
        get_test_library.client_disconnect(clean_l3_traffic=True)
        ...
        atn2 = ser_no[1].split(".")[2]
        print(f"antenuation-2 : {atn2}")
        val = [['modes: Auto'], ['pkts: MTU'], ['directions: DUT Receive'], ['traffic_types:TCP'],['bandw_options: AUTO'],
               ['spatial_streams: 2'], ['attenuator: ' + str(ser_no[1])],['attenuator2: 0'], ['attenuations: 0 100 210..+30..630'],
               ['attenuations2: 0 100 210..+30..630'], ['chamber: 0'],['tt_deg: 0']]
        if station:
            rvr_o, report_name = get_test_library.rate_vs_range_test(station_name=sta_name[0], mode=mode,
                                                                     download_rate="100%",
                                                                     duration='30000',
                                                                     instance_name="BRIDGE_RVR_TWOG_UL", vlan_id=vlan,
                                                                     dut_name=dut_name, raw_lines=val)
            entries = os.listdir("../reports/" + report_name + '/')
            print("entries", entries)
            print("Test Completed... Cleaning up Stations")
    ...
    def test_rvr_bridge_dl_wpa2_personal_5g(self, setup_configuration, get_test_library, num_stations,
                                          get_test_device_logs, get_dut_logs_per_test_case, check_connectivity):
        logging.info("Cleanup existing clients and traffic")
        chamber_view_obj, dut_name = get_test_library.chamber_view()
        get_test_library.client_disconnect(clean_l3_traffic=True)
        ...
        if station:
            time.sleep(3)
            rvr_o, report_name = get_test_library.rate_vs_range_test(station_name=sta_name[0], mode=mode,download_rate="100%",
                                                        duration='30000',instance_name="BRIDGE_RVR_FIVEG_DL",vlan_id=vlan,
                                                        dut_name=dut_name, raw_lines=val)
            entries = os.listdir("../reports/" + report_name + '/')
            print("entries", entries)
    ...
    @allure.testcase(url="https://telecominfraproject.atlassian.net/browse/WIFI-2499", name="WIFI-2499")
    def test_rvr_bridge_ul_wpa2_personal_5g(self, setup_configuration, get_test_library, num_stations,
                                         get_test_device_logs, get_dut_logs_per_test_case, check_connectivity):
        logging.info("Cleanup existing clients and traffic")
        chamber_view_obj, dut_name = get_test_library.chamber_view()
        get_test_library.client_disconnect(clean_l3_traffic=True)
        ...
        atn2 = ser_no[1].split(".")[2]
        print(f"antenuation-2 : {atn2}")
        val = [['modes: 802.11an-AC'], ['pkts: MTU'], ['directions: DUT Receive'], ['traffic_types:TCP'],
               ['bandw_options: AUTO'], ['spatial_streams: 2'], ['attenuator: ' + str(ser_no[1])], ['attenuator2: 0'],
               ['attenuations: 0 100 210..+30..540'], ['attenuations2: 0 100 210..+30..540'], ['chamber: 0'],
               ['tt_deg: 0']]
            ...
            if kpi in entries:
                ...
                if str(kpi_val) == "empty":
                    ...
                    if "FAIL" in pass_fail:
                        ...
                        logging.info("Test passed successfully")
                        allure.attach(name="Test Results", body="Throughput value :" + str(thrpt_val))
                        assert True
            else:
                logging.info("csv file does not exist, TEST FAILED, ")
                allure.attach(name="CSV Data", body="csv file does not exist")
