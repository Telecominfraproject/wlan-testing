...
class Test_RatevsRange_Nat(object):
    ...
    def test_rvr_nat_dl_wpa2_personal_2g(self, setup_configuration, get_test_library, num_stations,
        ...
        if station:
            rvr_o, report_name = get_test_library.rate_vs_range_test(station_name=sta_name[0], mode=mode, download_rate="100%",
                                                        duration='30000',instance_name="NAT_RVR_TWOG_DL",vlan_id=vlan,
    ...
    @allure.testcase(url="https://telecominfraproject.atlassian.net/browse/WIFI-2495", name="WIFI-2495")
    def test_rvr_nat_ul_wpa2_personal_2g(self, setup_configuration, get_test_library, num_stations,
                                      get_test_device_logs, get_dut_logs_per_test_case, check_connectivity):
        logging.info("Cleanup existing clients and traffic")
        chamber_view_obj, dut_name = get_test_library.chamber_view()
        get_test_library.client_disconnect(clean_l3_traffic=True)
        ...
        if station:
            rvr_o, report_name = get_test_library.rate_vs_range_test(station_name=sta_name[0], mode=mode,
                                                                     download_rate="100%",
                                                                     duration='30000', instance_name="NAT_RVR_TWOG_UL",
                                                                     vlan_id=vlan,
                                                                     dut_name=dut_name, raw_lines=val)
            entries = os.listdir("../reports/" + report_name + '/')
            print("entries", entries)
    ...
    def test_rvr_nat_dl_wpa2_personal_5g(self, setup_configuration, get_test_library, num_stations,
                                          get_test_device_logs, get_dut_logs_per_test_case, check_connectivity):
        logging.info("Cleanup existing clients and traffic")
        chamber_view_obj, dut_name = get_test_library.chamber_view()
        get_test_library.client_disconnect(clean_l3_traffic=True)
        ...
        if station:
            time.sleep(3)
            rvr_o, report_name = get_test_library.rate_vs_range_test(station_name=sta_name[0], mode=mode,download_rate="100%",
                                                        duration='30000',instance_name="NAT_RVR_FIVEG_DL",vlan_id=vlan,
                                                        dut_name=dut_name, raw_lines=val)
            entries = os.listdir("../reports/" + report_name + '/')
            print("entries", entries)
    ...
    @allure.testcase(url="https://telecominfraproject.atlassian.net/browse/WIFI-2499", name="WIFI-2499")
    def test_rvr_nat_ul_wpa2_personal_5g(self, setup_configuration, get_test_library, num_stations,
                                       get_test_device_logs, get_dut_logs_per_test_case, check_connectivity):
        logging.info("Cleanup existing clients and traffic")
        chamber_view_obj, dut_name = get_test_library.chamber_view()
        get_test_library.client_disconnect(clean_l3_traffic=True)
        ...
        if station:
            ...
            rvr_o, report_name = get_test_library.rate_vs_range_test(station_name=sta_name[0], mode=mode,
                                                                     download_rate="100%",
                                                                     duration='30000',
                                                                     instance_name="NAT_RVR_FIVEG_UL", vlan_id=vlan,
                                                                     dut_name=dut_name, raw_lines=val)
            entries = os.listdir("../reports/" + report_name + '/')
            print("entries", entries)
            logging.info("Test Completed... Cleaning up Stations")
