...
class TestMultiStaPerfBridge(object):
    ...
    #
    #     for i in range(2):
    #         lf_tools.set_radio_antenna("cli-json/set_wifi_radio", shelf, resource, lf_tools.twog_radios[i], 1)
    #         time.sleep(0.5)
    #         sta_ip = lf_test.Client_Connect_Using_Radio(ssid=ssid_name, passkey=profile_data["security_key"], radio=lf_tools.twog_radios[i], station_name=sta[i])
    #         if not sta_ip:
    #             print("test failed due to no station ip")
    #             assert False
    #         time.sleep(0.5)
    #     for i in range(4):
    #         lf_test.attenuator_modify(atten_sr[0], i, 100)
    #         time.sleep(0.5)
    #     for i in range(2):
    #         lf_test.attenuator_modify(atten_sr[1], i, 400)
    #         time.sleep(0.5)
    #
    #     #lf_tools.Chamber_View()
    ...
    #         time.sleep(0.5)
    #         if i >= 2:
    #             lf_test.attenuator_modify(atten_sr[1], i, 500)
    #             time.sleep(0.5)
    #
    #
    #     # lf_tools.Chamber_View()
    ...
    #                                     upload_rate="4Mbps", protocol="TCP-IPv4", duration="120000",
