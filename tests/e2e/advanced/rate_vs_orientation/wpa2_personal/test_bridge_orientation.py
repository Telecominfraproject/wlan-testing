...
class TestRatevsOrientationBridge(object):
    ...
    def test_client_tcp_dl_wpa2_personal_5g_11ac(self, lf_test, station_names_fiveg, create_lanforge_chamberview_dut,
        ...
        val = [['modes: 802.11an-AC'], ['pkts: MTU'], ['directions: DUT Transmit'], ['traffic_types:TCP'],
               ...
               ['attenuator2: ' + str(ser_no[1])], ['attenuations: 0 100 10..+30..540'],
               ['attenuations2: 0 100 10..+30..540'], ['chamber: DUT-Chamber'], ['tt_deg: 0..+30..150']] #210..+30..540
        ...
        if station:
            ...
            kpi = "kpi.csv"
            pass_value = {"strong":500, "medium":400, "weak":200}
            atn, deg = [0,10,21,24,27,30,33,36,39,42,45,48,51,54], [0,30,60,90,120,150,180,210,240,270,300,330]
            if kpi in entries:
                kpi_val = lf_tools.read_kpi_file(column_name=["numeric-score"], dir_name=report_name) #name="CSV Data", body="Throughput value : " + str(kpi_val)
                print(kpi_val)
                ...
                if str(kpi_val) == "empty":
                    ...
                    for i in pass_value:
                        ...
                        for j in range(start, len(kpi_val), len(atn)):
                            thrpt_val[f"{atn[start]}-{deg[count]}"] = kpi_val[j][0]
