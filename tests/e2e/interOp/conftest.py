...
def set_maverick_mode_all(request, get_configuration, get_apnos, fixtures_ver):
    ...
    for access_point_info in get_configuration['access_point']:
        ...
        # for i in range(15):
        connectivity_data = ap_ssh.run_generic_command(
        ...
        if "disconnected" in str(connectivity_data):
            ...
            connected = 1
        time.sleep(20)
        test_string = " Maverick"
        iwinfo_maverick = ap_ssh.get_iwinfo_maverick()

        # res = [ele for ele in iwinfo_maverick if (ele in test_string)]
        if (str(iwinfo_maverick).replace("\"","")).strip() == str(test_string).strip():
            print("SSID matched")
            allure.attach(name="iwinfo: ", body=str(iwinfo_maverick))
            #pytest.exit("Exiting Pytest")
        else:
            print("ssid didn't matched")
            allure.attach(name="iwinfo: ", body=str(iwinfo_maverick))
            #pytest.exit("IN else")

        def teardown_maverick_session():
            connectivity_data = ap_ssh.run_generic_command(cmd="/etc/init.d/ucentral start")
            time.sleep(15)
            iwinfo_maverick = ap_ssh.get_iwinfo_maverick()
            test_string = " Maverick"
            ...
            # res = [ele for ele in iwinfo_maverick if (ele in test_string)]
            if (str(iwinfo_maverick).replace("\"", "")).strip() != str(test_string).strip():
                print("SSID out of maverick confirmed")
                allure.attach(name="iwinfo: ", body=str(iwinfo_maverick))
                pytest.exit("Exiting Pytest Out of Maverick")
            else:
                print("SSID still Maverick confirmed")
                allure.attach(name="iwinfo: ", body=str(iwinfo_maverick))
                pytest.exit("Exiting Pytest In Maverick")
            # allure.attach(name="iwinfo: ", body=str(iwinfo))

            print("\nTeardown")

        request.addfinalizer(teardown_maverick_session)
        # redirector_data = ap_ssh.run_generic_command(cmd="cat /etc/ucentral/redirector.json")
        yield connected, redirector_data