...
class TestGTKrotationMulticastwithPowerSave(object):
    ...
    def test_gtk_rotation_multicast_with_power_save_fiveg(self,gtk_interval, get_configuration, lf_test,
        ...
        end_result_1,end_result=lf_test.verify_gtk_with_multicast_powersave(instantiate_profile, get_configuration, gtk_eap_interval,
                                                    ssid=ssid_name, password=security_key, security=security_type,
                                                    station_list=station_names_fiveg, test_dur=test_duration,
                                                    band=band)
        if end_result or end_result_1:
            assert False
        else:
            assert True
    ...
    #                                          instantiate_profile, get_controller_logs, get_ap_config_slots,
    #                                          get_lf_logs):
    def test_gtk_rotation_multicast_with_power_save_sixg(self, gtk_interval, get_configuration, lf_test,
                                                         station_names_ax, lf_tools,
                                                         run_lf,test_duration,
                                                         add_env_properties,
                                                         instantiate_profile, get_controller_logs, get_ap_config_slots,
                                                         get_lf_logs):
