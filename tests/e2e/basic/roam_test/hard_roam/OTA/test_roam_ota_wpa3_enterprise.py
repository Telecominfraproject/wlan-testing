...
@allure.sub_suite("wpa3_enterprise")
...
class TestRoamOTA(object):
    ...
    @pytest.mark.sixg
    @pytest.mark.tls
    def test_roam_6g_to_6g_sc_wpa3_eap_tls(self, get_target_object, get_test_library, get_lab_info, selected_testbed,
                                       radius_info):
        """
    ...
    @pytest.mark.both
    @pytest.mark.tls
    def test_roam_5g_to_2g_wpa3_eap_tls(self, get_target_object, get_test_library, get_lab_info, selected_testbed,
                                    radius_info):
        """
    ...
    @pytest.mark.both
    @pytest.mark.tls
    def test_roam_5g_to_6g_wpa3_eap_tls(self, get_target_object, get_test_library, get_lab_info, selected_testbed,
                                    radius_info):
        """
            Test Roaming between two APs, 5G & 6G, WPA3 Enterprise
            pytest -m "hard_roam_ota and fiveg and sixg and wpa3_enterprise and tls"
        """
        ...
        for ap in range(len(dut_list)):
            ...
            if str(ap_iwinfo) != "Error: pop from empty list":
                include_essid = config['interfaces'][0]["ssids"][0]["name"]
                logging.info(f"include_essid value:{include_essid}")
                re_obj = re.compile(
                    rf'([a-zA-Z0-9-]+)\s+ESSID: "{re.escape(include_essid)}".*?\s+Access Point:\s+([0-9A-Fa-f:]+).*?Channel:\s+('
                    r'\d+)\s+\(([\d.]+) GHz\)',
                    re.DOTALL
        ...
        ssid = config['interfaces'][0]["ssids"][0]["name"]
        pass_fail, message = True, "Test Passed"
        logging.info(f"bssid_list::{bssid_list}")
        logging.info(f"freqs_::{freqs_}")
        sixg_radio = list(get_test_library.get_radio_availabilities(num_stations_6g=1)[0].keys())[0]
        logging.info(f"sixg_radio from testcase:{sixg_radio}")
        try:
            pass_fail, message = get_test_library.roam_test(ap1_bssid=bssid_list[0], ap2_bssid=bssid_list[1],
    ...
    @pytest.mark.both
    @pytest.mark.tls
    def test_roam_2g_to_6g_wpa3_eap_tls(self, get_target_object, get_test_library, get_lab_info, selected_testbed,
                                       radius_info):
        """
        ...
        for ap in range(len(dut_list)):
            ...
            if ap == 1:
                ...
                channel_mode = "EHT" if "wifi7" in ap_mode else "HE"
                config['radios'] = [
                    {"band": "6G", "channel": 161, "channel-mode": channel_mode, "channel-width": 80, "country": "CA"}]
                config['interfaces'][0]["ssids"][0]["wifi-bands"] = ["6G"]
            logging.info(f"config:{config}")
            payload = {"configuration": json.dumps(config), "serialNumber": serial_number, "UUID": 2}
            uri = get_target_object.controller_library_object.build_uri(
    ...
    @pytest.mark.sixg
    @pytest.mark.ttls
    def test_roam_6g_to_6g_sc_wpa3_eap_ttls(self, get_target_object, get_test_library, get_lab_info, selected_testbed,
                                           radius_info):
        """
            Test Roaming between two APs, Same channel, 6G, WPA3 Enterprise
    ...
    @pytest.mark.both
    @pytest.mark.ttls
    def test_roam_5g_to_2g_wpa3_eap_ttls(self, get_target_object, get_test_library, get_lab_info, selected_testbed,
                                        radius_info):
        """
            Test Roaming between two APs, 2G & 5G, WPA3 Enterprise
    ...
    @pytest.mark.both
    @pytest.mark.ttls
    def test_roam_5g_to_6g_wpa3_eap_ttls(self, get_target_object, get_test_library, get_lab_info, selected_testbed,
                                        radius_info):
        """
            Test Roaming between two APs, 5G & 6G, WPA3 Enterprise
    ...
    @pytest.mark.both
    @pytest.mark.ttls
    def test_roam_2g_to_6g_wpa3_eap_ttls(self, get_target_object, get_test_library, get_lab_info, selected_testbed,
                                        radius_info):
        """
            Test Roaming between two APs, 2G & 6G, WPA3 Enterprise
