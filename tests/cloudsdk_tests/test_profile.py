...
class TestProfileCleanup(object):
    ...
    @pytest.mark.sanity(after='TestConnection')
            ...
            for i in setup_profile_data:
                for j in setup_profile_data[i]:
                    for k in setup_profile_data[i][j]:
                        instantiate_profile.delete_profile_by_name(profile_name=setup_profile_data[i][j][k]['profile_name'])
...
@pytest.mark.sanity(depends=['test_ap_manager_state'])
@pytest.mark.bridge
...
@pytest.mark.sanity(depends=['TestRfProfile'])
@pytest.mark.bridge
...
@pytest.mark.sanity(after='TestRadiusProfile')
@pytest.mark.ssid
@pytest.mark.bridge
...
#
#     @pytest.mark.sanity(order=9)
#     @pytest.mark.bridge
#     @pytest.mark.bridge_pp(order=4)
#     def test_equipment_ap_profile_bridge_mode(self, create_ap_profile_bridge):
