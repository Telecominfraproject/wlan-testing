...
class dut_lib_template:
    ...
    def setup_firmware(self):
        ...
        for i in range(len(self.device_under_tests_info)):
            try:
                output = self.get_dut_library_object().run_generic_command(cmd="show boot", idx=i)
                logging.info("current firmware version" + str(output))
                allure.attach(name="firmware url: ", body=str(self.device_under_tests_info[i]['firmware_version']))
                cmd = "upgrade " + str(self.device_under_tests_info[i]['firmware_version'])
                output = self.get_dut_library_object().run_generic_command(cmd=cmd, idx=i)
                logging.info("Upgrade command output" + str(output))
                upgrade_status.append([str(self.device_under_tests_info[i]['firmware_version'])])
            except Exception as e:
