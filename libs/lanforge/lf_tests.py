...
class RunTest:
    ...
    def dataplane(self, station_name=None, mode="BRIDGE", vlan_id=100, download_rate="85%", dut_name="TIP",
        ...
        if move_to_influx:
            report_name="../reports/" + \
                        self.dataplane_obj.report_name[0]['LAST']["response"].split(":::")[1].split("/")[-1]
            try:
                influx=CSVtoInflux(influx_host=self.influx_params["influx_host"],
                                   influx_port=self.influx_params["influx_port"],
                                   influx_org=self.influx_params["influx_org"],
        ...
        try:
            ...
            with open(filename, 'w') as f:
                lines=["bssid: " + str(bssid), 'rssi: ' + str(rssi), "rx rate: " + str(rx_rate),
                       "rx packets: " + str(rx_packets), "rx bytes: " + str(rx_bytes),
                       "connection time: " + str(cx_time)]
                for line in lines:
                    f.write(line)
                    f.write('\n')
    ...
    def hard_roam(self, run_lf, instantiate_profile, lf_tools, lf_reports, get_configuration, test=None, band=None,
                  num_sta=1,
    ...
    def layer3_traffic(self, ssid_num=8, band="2.4 Ghz", station_name=[]):
        ...
        for station_info in self.staConnect.resulting_stations:
            ...
            try:
                ...
                print(e)
            allure.attach(name=str(self.staConnect.resulting_stations[station_info]["interface"]['alias']),
                          body=data_table)
            data_table=""
            dict_table.clear()
            ...
            try:
                ...
                print(e)
            allure.attach(name="cx-" + str(self.staConnect.resulting_stations[station_info]["interface"]['alias']),
                          body=str(data_table))
            count+=1
        self.staConnect.stop()
        self.staConnect.cleanup()

    def rsn_test(self, mode="BRIDGE", security_mode="open", ssid="[BLANK]", passkey="[BLANK]", security="wpa2",
